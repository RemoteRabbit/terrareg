...
-- @param lockfile table Lockfile table containing version lock information
-- @return boolean True if write succeeded, false on error
...
-- @return table|nil Registry entries table, or nil if file doesn't exist or read fails
...
-- @return table|nil Index table, or nil if file doesn't exist or read fails
M.read_index = function()
	if not INDEX_PATH:exists() then
		log.debug("Index file does not exist")
		return
	end
	local ok, result = pcall(function()
	...
	if not ok then
		log.error("Failed to read index file: " .. tostring(result))
		return
	end
	return result
end
...
-- @return table|nil Lockfile table, or nil if file doesn't exist or read fails
